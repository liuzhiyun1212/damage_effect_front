...
import {
  ...
  listPartsManufactureChange,
} from "@/api/system/modifyData";
import * as echarts from "echarts";

export default {
  name: "FaultyPartsChange",
  data() {
    return {
      productChange: [],
      se: [],
  ...
  methods: {
    getX() {
      for (let index = 0; index < this.productChange.length; index++) {
        // if(!this.x.includes(this.productChange[index].modifyTime)){
    ...
    testChange() {
      listPartsManufactureChange().then((res) => {
