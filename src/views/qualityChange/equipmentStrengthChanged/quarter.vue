  ...
  <div>
    ...
    <div style="width: 100%; background: #d2e9ff; border-radius: 10px">
      ...
      <el-tooltip placement="top">
        <div slot="content">质量问题涉及装备使用强度的季度变更情况</div>
        <i
...
export default {
  ...
  methods: {
    ...
    dealData() {
      ...
      for (let k = 0; k < this.data12.length; k++) {
        for (let l = 0; l < this.equipment.length; l++) {
          if (this.data12[k].planeType === this.equipment[l].planeType) {
            ...
            let quarter =
              Math.floor((this.data12[k].date.slice(5, 7) - 1) / 3) + 1
            let flightHours = this.data12[k].flightHours
            for (let o = 0; o < this.equipment[l].strength.length; o++) {
              if (
                year === this.equipment[l].strength[o].year &&
                quarter === this.equipment[l].strength[o].quarter
              ) {
                flag = 1
      ...
      for (let x = 0; x < this.equipment.length; x++) {
        for (let y = 0; y < this.equipment[x].strength.length; y++) {
          this.equipment[x].strength[y].flightHours /= 3
        }
      }
      // 去空
    ...
    dealX() {
      for (let i = 0; i < this.equipment.length; i++) {
        for (let j = 0; j < this.equipment[i].strength.length; j++) {
          let flag = 0
          let t =
            this.equipment[i].strength[j].year +
            "年" +
            this.equipment[i].strength[j].quarter +
            "季度"
    ...
    //初始化 强度变更折线图
    init_Chart_quarterLine() {
      let quarterLine_chart = echarts.init(
        document.getElementById("quarterLine")
      )
      // echarts自适应
      window.addEventListener("resize", () => {
        quarterLine_chart.resize()
      })
      quarterLine_chart.setOption(this.set_Option_quarterLine())
    },
    // 初始化 强度变更折线图
    set_Option_quarterLine() {
      // 变更时间线折线图
      let option = {
        tooltip: {
          trigger: "axis",
